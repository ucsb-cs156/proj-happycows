...
describe("AdminDashboardPage", () => {
  test("renders dashboard title, sections, cards, and placeholders for a valid commons id", () => {
    ...
    const commonsIdStrong = screen.getByText(/^commons id:$/i, { selector: "strong" });
    expect(commonsIdStrong).toBeInTheDocument();
    expect(commonsIdStrong.closest("p")).toHaveTextContent(/commons id:\s*7/i);

    expect(screen.getByRole("heading", { name: /overview/i })).toBeInTheDocument();
    expect(screen.getByRole("heading", { name: /commons details/i })).toBeInTheDocument();
    expect(screen.getByRole("heading", { name: /participation metrics/i })).toBeInTheDocument();
    expect(screen.getByRole("heading", { name: /farmer cow distribution/i })).toBeInTheDocument();
    expect(screen.getByRole("heading", { name: /trends over time/i })).toBeInTheDocument();
    expect(screen.getByRole("heading", { name: /additional insights/i })).toBeInTheDocument();
    ...
    expect(screen.getByText(/^name:$/i, { selector: "strong" })).toBeInTheDocument();
    expect(screen.getByText(/^status:$/i, { selector: "strong" })).toBeInTheDocument();
    expect(screen.getByText(/^start date:$/i, { selector: "strong" })).toBeInTheDocument();

    expect(screen.getByText(/^active farmers:$/i, { selector: "strong" })).toBeInTheDocument();
    expect(screen.getByText(/^inactive farmers:$/i, { selector: "strong" })).toBeInTheDocument();
    expect(screen.getByText(/^avg cows per farmer:$/i, { selector: "strong" })).toBeInTheDocument();

    expect(
      screen.getByText(/histogram \/ distribution of cows per farmer will go here/i)
    ).toBeInTheDocument();
    ...
    expect(
      screen.getAllByText(/chart placeholder \(x-axis:\s*days since commons start\)/i).length
  ...
  test("supports different ids (ensures id is not hard-coded)", () => {
    ...
    const commonsIdStrong = screen.getByText(/^commons id:$/i, { selector: "strong" });
    expect(commonsIdStrong.closest("p")).toHaveTextContent(/commons id:\s*123/i);
    expect(commonsIdStrong.closest("p")).not.toHaveTextContent(/commons id:\s*7/i);
  });
});