...
describe("PagedAnnouncementTable tests", () => {
    ...
    test("for page content render mutiple time", async () => {
        ...
        const mockUseParams2 = jest.spyOn(require('react-router-dom'), 'useParams');
        mockUseParams2.mockReturnValue({ commonsId: 1 });
        ...
        await waitFor(() => {
            expect(screen.getByText("Future announcement")).toBeInTheDocument();
        })
    });
});