...
describe('CommonsTable sorting and extra getSortableValue branches', () => {
  test('getSortableValue covers milkPrice, startingBalance, lastDate, degradationRate', () => {
    ...
    expect(getSortableValue(row, 'commons.lastDate')).toBe('2023-01-02T03:04:05Z');
  ...
  test('component sorting handles null vs non-null for defaults (no crash)', async () => {
    ...
    const commons = [
      { commons: { /* id missing */ name: null, cowPrice: '5' }, totalCows: '2' },
      { commons: { id: 2, name: 'Bravo', cowPrice: '10' }, totalCows: '3' },
      { commons: { /* id missing */ name: null, cowPrice: '' }, totalCows: '1' },
    ];
  ...
  test('component numeric sorting by milkPrice ascending and descending', async () => {
    ...
    const names = screen.getAllByTestId(/CommonsTable-card-\d+-name/).map(n => n.textContent);
    expect(names).toEqual(['Alpha', 'Charlie', 'Bravo']);

    // toggle direction to desc
    await userEvent.click(screen.getByTestId('CommonsTable-sort-direction-toggle'));

    const namesDesc = screen.getAllByTestId(/CommonsTable-card-\d+-name/).map(n => n.textContent);
