...
describe('ManageCowsModal', () => {
  ...
  test('calls onBuy with correct arguments when buy button is clicked (0 argument)', async () => {
    const number2 = 0;
    render(
    ...
    await waitFor(()=>expect(mockOnBuy).toHaveBeenCalledWith(userCommonsFixtures.oneUserCommons[0], number2));
  });
  ...
  test('Check styling of modal components (Sell)', () => {
    render(
        <ManageCowsModal isOpen={true} onClose={mockOnClose} message="Sell" setNumber={mockSetNumber} />
    );
