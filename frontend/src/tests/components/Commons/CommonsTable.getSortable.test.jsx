...
import { getSortableValue, computeEffectiveCapacity } from '../../../../src/main/components/Commons/CommonsTable'
...
describe('CommonsTable getSortableValue exhaustive tests', () => {
  ...
  it('handles string/date keys and returns defaults for missing', () => {
    const cp = { commons: { name: 'Farm A', startingDate: '2020-01-02T00:00:00Z', lastDate: null } }
    expect(getSortableValue(cp, 'commons.name')).toBe('Farm A')
    // startingDate is returned as-is (string)
    expect(getSortableValue(cp, 'commons.startingDate')).toBe('2020-01-02T00:00:00Z')
  ...
  it('computes effectiveCapacity correctly in all branches', () => {
    const withExplicit = { effectiveCapacity: 123, commons: { capacityPerUser: 2 }, totalUsers: 10 }
    expect(computeEffectiveCapacity(withExplicit)).toBe(123)

    const computed = { effectiveCapacity: null, commons: { capacityPerUser: 3 }, totalUsers: 4 }
    expect(computeEffectiveCapacity(computed)).toBe(12)

    const unknown = { effectiveCapacity: null, commons: { capacityPerUser: null }, totalUsers: null }
