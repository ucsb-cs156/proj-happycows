...
describe('CommonsTable extra helpers coverage', () => {
  ...
  test('computeEffectiveCapacity uses effectiveCapacity or falls back', () => {
    const withEff = { effectiveCapacity: 42, commons: { capacityPerUser: '2' }, totalUsers: '10' };
    expect(computeEffectiveCapacity(withEff)).toBe(42);

    const fallback = { effectiveCapacity: undefined, commons: { capacityPerUser: '3' }, totalUsers: '4' };
    expect(computeEffectiveCapacity(fallback)).toBe(12);

    const missing = { effectiveCapacity: undefined, commons: { capacityPerUser: null }, totalUsers: null };
    expect(computeEffectiveCapacity(missing)).toBeNull();
  });

  test('getSortableValue handles booleans and numeric carry cap', () => {
    const row = { commons: { showLeaderboard: true, showChat: false, carryingCapacity: '10' }, totalCows: '7' };
