...
describe("StudentsEditForm tests", () => {
    const queryClient = new QueryClient();

    const expectedHeaders = ["Perm", "Last Name", "First and Middle Name", "Email"];
    const testId = "StudentsEditForm";
    ...
    test("renders correctly when passing in initialContents", async () => {
        render(
            <QueryClientProvider client={queryClient}>
                <Router>
                    <StudentsEditForm initialContents={studentsFixtures.oneStudent} />
                </Router>
            </QueryClientProvider>
        );
    ...
    test("that the correct validations are performed", async () => {
        ...
        expect(
            screen.queryByText(/The Course ID is required./),
