...
export default function AdminListGamePageV2() {
  ...
  const {
    ...
    ["/api/game/allplus"],
    { method: "GET", url: "/api/game/allplus" },
    [],
  );
  // Stryker restore  all

  const sortedGame = [...game].sort((a, b) => b.game.id - a.game.id);

  const filteredGame = sortedGame.filter((c) =>
    c.game.name.toLowerCase().includes(query.toLowerCase()),
  );
  ...
  useEffect(() => {
    ...
    if (!focusId || isFetching || game.length === 0) {
      return;
    }
    scrollToFocusTarget({ targetId: focusId });
    navigate(location.pathname, { replace: true });
  }, [focusId, isFetching, game, navigate, location.pathname]);
  ...
  return (
    <BasicLayout>
      <div className="pt-2">
          ...
          <Col md={6}>
            <div className="search-container">
              <Form.Control
                ...
                data-testid="AdminListGamePage-Search"
        ...
        >
          {filteredGame &&
            filteredGame.map((common) => (
              <div key={common.game.id} id={String(common.game.id)}>
                <AdminGameCard commonItem={common} currentUser={currentUser} />
              </div>
            ))}
        </Container>
      </div>
