...
import HealthUpdateStrategiesDropdown from "main/components/Game/HealthStrategiesUpdateDropdown";
import CourseSelectDropdown from "main/components/Courses/CourseSelectDropdown";

function GameForm({ initialGame, submitAction, buttonLabel = "Create" }) {
  let modifiedGame = initialGame ? { ...initialGame } : {}; // make a shallow copy of initialGame

  if (modifiedGame.startingDate) {
    modifiedGame.startingDate = modifiedGame.startingDate.split("T")[0];
  }

  if (modifiedGame.lastDate) {
    modifiedGame.lastDate = modifiedGame.lastDate.split("T")[0];
  }
  ...
  const {
    ...
    // modifiedGame is guaranteed to be defined (initialGame or {})
    { defaultValues: modifiedGame },
  );
  // Stryker restore all
  ...
  // Stryker disable all
  const { data: defaultValuesData } = useBackend("/api/game/defaults", {
    method: "GET",
    url: "/api/game/defaults",
  });
  // Stryker restore all

  // Stryker disable all
  useEffect(() => {
    if (defaultValuesData && !initialGame) {
      const {
        startingBalance,
        cowPrice,
        milkPrice,
  ...
  }, [defaultValuesData, initialGame, reset]);
  // Stryker restore all

  const testid = "GameForm";
  const curr = new Date();
  ...
  return (
    <Form onSubmit={handleSubmit(submitAction)}>
      ...
      <Row>
        <Col md={6}>
          <CourseSelectDropdown
            formName="courseId"
            displayName="Course"
            initialValue={modifiedGame.courseId}
            register={register}
      ...
      <Row className="mb-5">
        <Button
          type="submit"
          data-testid="GameForm-Submit-Button"
