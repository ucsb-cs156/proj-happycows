...
public class CommonStatsCSVHelper {
   ...
   * method call from jacoco coverage, but we can exclude the entire method by name in the pom.xml
   *
   * @param out
   */
  public static void flush_and_close_noPitest(ByteArrayOutputStream out, CSVPrinter csvPrinter)
      throws IOException {
    csvPrinter.flush();
    csvPrinter.close();
    out.flush();
  ...
  public static ByteArrayInputStream toCSV(Iterable<CommonStats> stats) throws IOException {
    ...
    List<String> headers = Arrays.asList("id", "commonsId", "numCows", "avgHealth", "createDate");
