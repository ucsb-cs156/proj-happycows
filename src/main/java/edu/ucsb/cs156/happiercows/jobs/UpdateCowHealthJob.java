...
public class UpdateCowHealthJob implements JobContextConsumer {
  ...
  @Getter private CommonsPlusBuilderService commonsPlusBuilderService;
  ...
  public void accept(JobContext ctx) throws Exception {
    ...
    Iterable<CommonsPlus> allCommonsPlus =
        commonsPlusBuilderService.convertToCommonsPlus(allCommons);
  ...
  public static void runUpdateJobInCommons(
      Commons commons,
      CommonsPlus commonsPlus,
    ...
    ctx.log(
        "Commons "
            + commons.getName()
            + ", degradationRate: "
            + commons.getDegradationRate()
            + ", effectiveCapacity: "
            + commonsPlus.getEffectiveCapacity());

    int numUsers =
        commonsRepository
            .getNumUsers(commons.getId())
            .orElseThrow(
                () -> new RuntimeException("Error calling getNumUsers(" + commons.getId() + ")"));
    ...
    Integer totalCows =
        commonsRepository
            .getNumCows(commons.getId())
            .orElseThrow(
                () -> new RuntimeException("Error calling getNumCows(" + commons.getId() + ")"));

    var isAboveCapacity = totalCows > carryingCapacity;
    var cowHealthUpdateStrategy =
        isAboveCapacity
            ? commons.getAboveCapacityHealthUpdateStrategy()
            : commons.getBelowCapacityHealthUpdateStrategy();
    ...
    for (UserCommons userCommons : allUserCommons) {
      ...
      ctx.log(
          "User: "
              + user.getFullName()
              + ", numCows: "
              + userCommons.getNumOfCows()
              + ", cowHealth: "
              + userCommons.getCowHealth());
