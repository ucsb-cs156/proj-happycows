...
/**
 * This job computes the stats for all games in progress and creates one new row in the CommonsStats
 * table for each commons. It uses the Average Cow Health Service to compute the cowhealth for each
 * commons.
 */
@AllArgsConstructor
public class RecordCommonStatsJob implements JobContextConsumer {

  @Getter private CommonStatsService commonStatsService;
  ...
  public void accept(JobContext ctx) throws Exception {
    ...
    for (Commons commons : allCommons) {
      ...
      ctx.log(
          String.format(
              "CommonStats %d for commons id=%d (%s) finished.",
              commonStats.getId(), commons.getId(), commons.getName()));
    }
