...
public class MilkTheCowsJob implements JobContextConsumer {
  ...
  @Getter private ProfitRepository profitRepository;
  ...
  /**
   * This method performs the function of milking the cows for a single userCommons. It is a public
   * method only so it can be exposed to the unit tests
   *
  ...
  public static void milkCows(
    ...
    ctx.log(
        "User: "
            ...
            + ", totalWealth: "
            + formatDollars(userCommons.getTotalWealth()));
    ...
    ctx.log(
        "Profit for user: "
            + user.getFullName()
            + " is: "
            + formatDollars(profitAmount)
            + ", newWealth: "
            + formatDollars(newWeath));
  }
