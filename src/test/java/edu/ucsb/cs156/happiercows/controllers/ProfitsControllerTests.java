...
public class ProfitsControllerTests extends ControllerTestCase {
  ...
  @MockBean ProfitRepository profitRepository;
  ...
  UserCommons uc1 = UserCommons.builder().user(user).commons(commons).build();
  ...
  public void get_profits_all_commons_nonexistent_using_commons_id() throws Exception {
    MvcResult response =
        mockMvc
            .perform(get("/api/profits/all/commonsid?commonsId=2").contentType("application/json"))
  ...
  public void get_profits_all_commons_using_commons_id() throws Exception {
    ...
    when(userCommonsRepository.findByCommonsIdAndUserId(2L, 1L))
        .thenReturn(Optional.of(expectedUserCommons));

    MvcResult response =
        mockMvc
            .perform(get("/api/profits/all/commonsid?commonsId=2"))
            .andDo(print())
    ...
    List<Profit> actualProfits =
        objectMapper.readValue(responseString, new TypeReference<List<Profit>>() {});
  ...
  public void admin_get_profits_all_commons_nonexistent_using_commons_id() throws Exception {
    MvcResult response =
        mockMvc
            .perform(get("/api/profits/all?userId=1&commonsId=2").contentType("application/json"))
            .andExpect(status().isNotFound())
            .andReturn();
  ...
  public void admin_get_profits_all_commons_using_commons_id() throws Exception {
    ...
    MvcResult response =
        mockMvc
            .perform(get("/api/profits/all?userId=1&commonsId=2"))
            .andDo(print())
            .andExpect(status().isOk())
            .andReturn();
  ...
  @WithMockUser(roles = {"USER"})
  @Test
  public void get_profits_all_commons_nonexistent_using_commons_id_with_pagination()
      throws Exception {
    MvcResult response =
        mockMvc
            .perform(
                get("/api/profits/paged/commonsid?commonsId=2").contentType("application/json"))
            .andExpect(status().isNotFound())
            .andReturn();
  ...
  public void get_profits_all_commons_using_commons_id_with_pagination() throws Exception {
    ...
    MvcResult response =
        mockMvc
            .perform(get("/api/profits/paged/commonsid?commonsId=2&pageNumber=0&pageSize=7"))
            .andDo(print())
            .andExpect(status().isOk())
            ...
            .andExpect(
                jsonPath("$.content[0].amount")
                    .value(p1.getAmount())) // Example check for an attribute
            .andReturn();
  ...
  public void get_profits_all_commons_using_commons_id_with_pagination_2() throws Exception {
    ...
    MvcResult response =
        mockMvc
            .perform(get("/api/profits/paged/commonsid?commonsId=2&pageNumber=1&pageSize=2"))
            .andDo(print())
            .andExpect(status().isOk())
            .andExpect(jsonPath("$.content").isArray())
            .andReturn();
