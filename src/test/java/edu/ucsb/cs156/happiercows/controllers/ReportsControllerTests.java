...
public class ReportsControllerTests extends ControllerTestCase {
  ...
  @MockBean ReportLineRepository reportLineRepository;
  ...
  private User user =
      User.builder().id(42L).fullName("Chris Gaucho").email("cgaucho@example.org").build();
  ...
  public void get_all_report_headers() throws Exception {
    ...
    MvcResult response =
        mockMvc.perform(get("/api/reports")).andDo(print()).andExpect(status().isOk()).andReturn();

    verify(reportRepository, times(1)).findAll(any());

    String responseString = response.getResponse().getContentAsString();
    List<Report> actualReports =
        objectMapper.readValue(responseString, new TypeReference<List<Report>>() {});
  ...
  public void get_specific_report_header() throws Exception {
    ...
    MvcResult response =
        mockMvc
            .perform(get("/api/reports/byReportId?reportId=432"))
            .andDo(print())
    ...
    String responseString = response.getResponse().getContentAsString();
    Optional<Report> actualReports =
        objectMapper.readValue(responseString, new TypeReference<Optional<Report>>() {});
  ...
  public void get_reports_headers_commonId() throws Exception {
    ...
    MvcResult response =
        mockMvc
            .perform(get("/api/reports/headers?commonsId=17"))
            .andDo(print())
            .andExpect(status().isOk())
            .andReturn();
  ...
  public void get_reports_lines_commonId() throws Exception {
    ...
    MvcResult response =
        mockMvc
            .perform(get("/api/reports/lines?reportId=432"))
            .andDo(print())
            .andExpect(status().isOk())
            .andReturn();
    ...
    String responseString = response.getResponse().getContentAsString();
    List<ReportLine> actualLines =
        objectMapper.readValue(responseString, new TypeReference<List<ReportLine>>() {});
  ...
  public void test_get_csv() throws Exception {
    ...
    MvcResult response =
        mockMvc
            .perform(get("/api/reports/download?reportId=432"))
            .andDo(print())
            .andExpect(status().isOk())
            .andReturn();
