...
public class CommonsTests {
  ...
  void test_gameInProgress_true() throws Exception {
    assertEquals(
        true, Commons.builder().startingDate(start).lastDate(end).build().gameInProgress());
  }

  @Test
  void test_gameInProgress_not_started() throws Exception {
    assertEquals(
        false, Commons.builder().startingDate(start2).lastDate(end).build().gameInProgress());
  }

  @Test
  void test_gameInProgress_ended() throws Exception {
    assertEquals(
        false, Commons.builder().startingDate(start).lastDate(end2).build().gameInProgress());
  }
}
