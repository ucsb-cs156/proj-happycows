...
public class AverageCowHealthServiceTests {
  ...
  @Autowired AverageCowHealthService averageCowHealthService;
  ...
  private User user2 =
      User.builder().id(43L).fullName("John Doe").email("jdoe@example.org").build();
  ...
  void test_getAverageCowHealthOneUser() {
    ...
    when(commonsRepository.getNumCows(commons.getId()))
        .thenReturn(Optional.of(Integer.valueOf(20)));
  ...
  void test_getAverageCowHealthMultipleUsers() {
    ...
    when(commonsRepository.getNumUsers(commons.getId()))
        .thenReturn(Optional.of(Integer.valueOf(1)));
    when(commonsRepository.getNumCows(commons.getId()))
        .thenReturn(Optional.of(Integer.valueOf(120)));
    when(userRepository.findById(42L)).thenReturn(Optional.of(user1));
