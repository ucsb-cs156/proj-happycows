...
public class CommonsPlusBuilderServiceTests {
  ...
  private CommonsPlus commonsPlus =
      CommonsPlus.builder().commons(commons).totalCows(10).totalUsers(5).build();
  ...
  void test_convertToCommonsPlus() {
    when(commonsRepository.getNumCows(17L)).thenReturn(Optional.of(10));
    when(commonsRepository.getNumUsers(17L)).thenReturn(Optional.of(5));
    Iterable<CommonsPlus> commonsPlusIterable =
        commonsPlusBuilderService.convertToCommonsPlus(Arrays.asList(commons));
