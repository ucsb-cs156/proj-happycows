...
public class SetCowHealthJobTests extends JobTestCase {
  ...
  void error_msg_when_no_commons_found() throws Exception {
    ...
    // Act
    SetCowHealthJob setCowHealthJob =
        new SetCowHealthJob(117L, 2.0, commonsRepository, userCommonsRepository, userRepository);
  ...
  void test_updating_to_new_values_for_multiple() throws Exception {
    ...
    var userCommonsList = Arrays.asList(getUserCommons(), getUserCommons(), getUserCommons());
    ...
    // Act
    SetCowHealthJob setCowHealthJob =
        new SetCowHealthJob(117, 2, commonsRepository, userCommonsRepository, userRepository);
    setCowHealthJob.accept(ctx);
    ...
    assertEquals(expected, jobStarted.getLog());
    userCommonsList.forEach(
        userCommons -> assertEquals(newUserCommons.getCowHealth(), userCommons.getCowHealth()));
  }
}
