...
class HealthUpdateStrategyListTests {
  ...
  void createdValueHasDefaults() {
    assertEquals(
        CowHealthUpdateStrategies.DEFAULT_ABOVE_CAPACITY.name(), value.getDefaultAboveCapacity());
    assertEquals(
        CowHealthUpdateStrategies.DEFAULT_BELOW_CAPACITY.name(), value.getDefaultBelowCapacity());
  }
